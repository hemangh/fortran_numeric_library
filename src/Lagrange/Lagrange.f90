...
!********************************************************************************
  ...
  implicit none
  private
  public lagrange_interpolate_1d, LaGrange
...
!     END IF
     ...
     nc_wt(1:n_x,int) = 0.d0    ! Compute the integration weight for the integration grid.
     DO pt = 1, n_x
        DO gpt = 1, ny_i
           nc_wt(pt,int) = nc_wt(pt,int) + p(gpt,pt) * wt(gpt)
        END DO
...
subroutine lagrange_interpolate_1d (n_x, x, fx, n_y , y, fy)

  INTEGER  , intent(in)                                   :: n_x
  REAL(idp), intent(in),            DIMENSION(:)          :: x
  REAL(idp), intent(in),            DIMENSION(:)          :: fx
  INTEGER  , intent(in)                                   :: n_y
  REAL(idp), intent(in),            DIMENSION(:)          :: y
  REAL(idp), intent(out),           DIMENSION(:)          :: fy
 
  type(LaGrange)                                          :: lgr
  REAL(idp), allocatable,            DIMENSION(:,:)       :: p
  !REAL(idp),  OPTIONAL, DIMENSION(:,:)       :: dp
  !REAL(idp),  OPTIONAL, DIMENSION(:,:)       :: ddp
  LOGICAL,              DIMENSION(2)         :: prnt_lagrange
  prnt_lagrange = .false.
  allocate(p(n_y,n_x))!,dp(),ddp())
  
  call lgr%Lagrange_Polynomials(p=p,x=x,y=y,prnt_lagrange=prnt_lagrange)
